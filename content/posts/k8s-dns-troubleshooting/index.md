...
这里我们发现了问题的根源：`core42.co` 域名在 Azure Private DNS Zones 中被定义了！这就解释了为什么集群内的 DNS 服务器无法解析这个域名。
...
1. 如果这个域名确实需要通过公网访问：
   ...
   - 删除 Azure Private DNS Zone 中的 `core42.co` 记录
   - 或者只保留特定的子域名（如 `internal.core42.co`）用于内部服务
2. 如果这是内部服务：
